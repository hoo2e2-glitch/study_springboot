...
public class AuthApi {
    ...
    public ResponseEntity<ApiResponseDTO> login(@RequestBody MemberDTO memberDTO, HttpServletResponse response) {
        ...
        ResponseCookie accessTokenCookie = ResponseCookie.from("accessToken", jwtTokenDTO.getAccessToken())
                .httpOnly(true) // XSS 공격차단
                .sameSite("Lax") // CSRF 공격차단
                .path("/")
        ...
        return ResponseEntity.status(HttpStatus.OK)
                ...
                .header(HttpHeaders.SET_COOKIE, refreshTokenCookie.toString(), refreshTokenCookie.toString())
    ...
    @PostMapping("/refresh")
    public ResponseEntity<ApiResponseDTO> me(@CookieValue(name = "refreshToken", required = false) String refreshToken)
    {
        ...
        return ResponseEntity.status(HttpStatus.OK)
                ...
                .body(ApiResponseDTO.of(true ,"토큰 재발금 완료"));
